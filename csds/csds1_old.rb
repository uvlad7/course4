...
class Vigenere
  ...
  def self.modufy(text, key, alphabet, magic_param)
    ...
    text.chars.map.with_index do |char, index|
      alphabet.include?(char) ? alphabet.advance(char, offset[index % key_len]) : ' '
    end.join
  end
end
...
class Kasiski
  def self.frequencies(text, alphabet)
    ...
    loop do
      ...
      (0..(text.length - n)).each do |i|
        ...
        result[gr] = { count: result[gr][:count] + 1, pos: (result[gr][:pos] || []).push(i) }
        has_repeats = true if result[gr][:count] == 2
      end
      break unless has_repeats
      n += 1
    end
    result.delete_if { |_key, value| value[:count] == 1 }.map do |key, value|
  ...
  def self.key_len(frequencies, threshold)
    freq_threshold = (frequencies.values.max * threshold).to_f / 100
    frequencies.reject { |_k, v| v < freq_threshold }.keys.reduce(&:gcd)
  end
...
class App
  ...
  def key_experiment(alphabet, text_len)
    ...
    thresholds = (0.5..50).step(2.5).to_a
    key_lens = [2, 3, 6, 10, 15, 20, 25, 35, 50, 100]

    success = Array.new(key_lens.size) { Array.new(thresholds.size) { 0 } }
    progressbar = ProgressBar.create(total: key_lens.size * thresholds.size * 10, title: 'Key experiment', output: $stderr)
    key_lens.each_with_index do |key_len, i|
      10.times do
        ...
        thresholds.each_with_index do |threshold, j|
          success[i][j] += 1 if Vigenere.decrypt(enc_text, Kasiski.key(enc_text, Kasiski.key_len(frequencies, threshold), alphabet), alphabet) == text
          progressbar.increment
        end
      end
    end

    xs, ys = np.meshgrid(np.array(thresholds), np.array(key_lens))
    ax.plot_wireframe(xs, ys, np.array(success), color: 'green')
    pyplot.show
  end
  ...
  def text_experiment(alphabet, key_len)
    ax = pyplot.figure.add_subplot(111, projection: '3d')
    ax.set_xlabel('Threshold, %')
    ax.set_ylabel('Text len')
    ax.set_zlabel('Result')

    thresholds = (0.5..50).step(2.5).to_a
    text_lens = [200, 300, 600, 1000, 1500, 2000, 2500, 3500, 5000, 10000]

    success = Array.new(text_lens.size) { Array.new(thresholds.size) { 0 } }
    progressbar = ProgressBar.create(total: text_lens.size * thresholds.size * 10, title: 'Text experiment', output: $stderr)
    ...
    xs, ys = np.meshgrid(np.array(thresholds), np.array(text_lens))
    ax.plot_wireframe(xs, ys, np.array(success), color: 'green')
    pyplot.show
  end
end
